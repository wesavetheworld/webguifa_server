...
var lobby = {
  ...
  updateGameList: function(gameList) {
    ...
    jQuery.each(gameList, function(index, game) {
        ...
        actionjoin = [];
        if (game.is_invitee || (!game.is_participant && !game.is_invitee &&
                game.status != lobby.GAME_STATUS_ACTIVE)) {
          if (game.is_invitee) {
          ...
          actionjoin.push({id: "joinGame", label: "加入"});
          jQuery.each(actionjoin, function(index, item) {
            if (index > 0) {
              content += "&nbsp;";
            }
            content += (game.opponent1 ? game.opponent1 : ('<a class="btn" href="#" key="' + game.key +
              '" id="' + item.id + '" seat="' + 1 + '">' + item.label + "</a>")) + "</td><td>";
            if (game.game_type == 2) {
              content += (game.ally? game.ally: ('<a class="btn" href="#" key="' + game.key +
                '" id="' + item.id + '" seat="' + 2 + '">' + item.label + "</a>")) + "</td><td>";
              content += (game.opponent2 ? game.opponent2 : ('<a class="btn" href="#" key="' + game.key +
                '" id="' + item.id + '" seat="' + 3 + '">' + item.label + "</a>")) + "</td><td>";
            } else {
              content += "关闭</td><td>关闭</td><td>";
            }
          });
        } else {
          content += (game.opponent1 ? game.opponent1 : "打开") + "</td><td>";
          if (game.game_type == 2) {
            content += (game.ally ? game.ally : "打开") + "</td><td>" +
              (game.opponent2 ? game.opponent2 : "打开") + "</td><td>";
          } else {
            content += "关闭</td><td>关闭</td><td>";
          }
        }
        content += (game.time_limit ?  (game.time_limit + " 分钟") : "无") +
          "</td><td>";
        ...
        if (game.is_participant || game.is_invitee) {
          ...
          if (game.is_invitee) {
          ...
          } else if (game.is_creator) {
            if (game.status == lobby.GAME_STATUS_READY) {
              action.push({id: "goGame", label: "开始"});
            ...
            if (!lobby.ignoreMap[game.key] && !blitz.dialogActive() &&
              ...
              lobby.displayEnterDialog(game.creator, game.key);
            }
          }
          if (game.can_delete) {
  ...
  goGame: function(gameKey) {
    var options = {
      url: "/game_ajax/" + gameKey + "/start",
      data: {gameKey: gameKey},
      type: "POST",
      success: blitz.clickHandlers.goGame(gameKey),
      error: function() {
        alert("失败了");
      }
    }
  ...
  joinGame: function(gameKey, seat) {
    var options = {
      ...
      data: {seat: seat},
      type: "POST",
      success: lobby.forceRefresh, // function() {lobby.goGame(gameKey); },
      error: function() {
        alert("无法加入对局");
        lobby.forceRefresh();
      }
  ...
  updatePlayerList: function(playerList) {
    ...
    $("#lobbyStatus").html("大厅<br/>" +
         playerList.length + " 位棋手在大厅中");
